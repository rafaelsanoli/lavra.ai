...
export default function SegurosPage() {
  ...
  const getPrioridadeColor = (prioridade: string) => {
    ...
    return colors[prioridade] || 'bg-gray-500/20 text-gray-400 border-gray-500/50'
  }
    ...
    <DashboardLayout>
      ...
      >
        ...
        <div className="flex gap-3">
          <button className="px-4 py-2 bg-white/5 hover:bg-white/10 backdrop-blur-xl border border-white/10 rounded-xl text-white transition-all flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        >
          <div className="flex items-center justify-between">
            <div>
              <p className="text-gray-400 text-sm">Taxa de Sinistralidade</p>
              <h3 className="text-2xl font-bold text-white mt-1">
        ...
        >
          ...
          <div className="flex items-center gap-2 mt-4 text-sm">
            <span className="text-gray-400">Dedução IR sobre prêmios</span>
          </div>
        </motion.div>
      </div>
              ...
              >
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                  <div>
                    ...
                    <p className="text-lg font-bold text-white">{formatarMoeda(apolice.cobertura)}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    <p className="text-sm text-gray-400">Vigência</p>
                    <p className="text-sm text-white">
                      {formatarData(apolice.vigenciaInicio)} - {formatarData(apolice.vigenciaFim)}
                    </p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-400">Franquia</p>
                    <p className="text-lg font-bold text-white">{apolice.franquia}%</p>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap gap-4">
                  {apolice.culturas.length > 0 && (
                    <div>
                      <p className="text-sm text-gray-400 mb-1">Culturas</p>
                      <div className="flex gap-2">
                  ...
                  <div>
                    <p className="text-sm text-gray-400 mb-1">Riscos Cobertos</p>
            ...
            {mockSegurosData.cotacoes.map((cotacao) => (
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      <h3 className="text-xl font-bold text-white">{cotacao.seguradora}</h3>
                    ...
                    <p className="text-gray-400">{cotacao.tipo}</p>
                    <p className="text-sm text-gray-500 mt-1">
                      Válida até: {formatarData(cotacao.dataValidade)}
                    </p>
                  </div>
                  <button className="px-4 py-2 bg-gradient-to-r from-brand-600 to-green-600 hover:from-brand-500 hover:to-green-500 rounded-xl text-white font-medium transition-all">
                    Contratar
                  </button>
                </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                  <div>
                    <p className="text-sm text-gray-400">Cobertura Proposta</p>
                    <p className="text-lg font-bold text-white">{formatarMoeda(cotacao.cobertura)}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-400">Prêmio Estimado</p>
                    <p className="text-lg font-bold text-brand-400">{formatarMoeda(cotacao.premioEstimado)}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-400">Vigência</p>
                    <p className="text-lg font-bold text-white">{cotacao.vigenciaMeses} meses</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-400">Franquia</p>
                    <p className="text-lg font-bold text-white">{cotacao.franquia}%</p>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap gap-4">
                  ...
                  <div>
                    <p className="text-sm text-gray-400 mb-1">Coberturas</p>
                    <div className="flex flex-wrap gap-2">
            ...
            {mockSegurosData.sinistros.map((sinistro) => (
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      <h3 className="text-xl font-bold text-white">{sinistro.tipo}</h3>
                    ...
                    <p className="text-gray-400">Apólice: {sinistro.apolice}</p>
                    <p className="text-sm text-gray-500 mt-1">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div>
                    <p className="text-sm text-gray-400">Área Afetada</p>
                    <p className="text-lg font-bold text-white">{sinistro.areaAfetada} ha</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-400">Perda Estimada</p>
                    <p className="text-lg font-bold text-red-400">{formatarMoeda(sinistro.perdaEstimada)}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-400">Indenização</p>
                    <p className="text-lg font-bold text-green-400">
                      {sinistro.indenizacaoPaga > 0 ? formatarMoeda(sinistro.indenizacaoPaga) : '-'}
                    </p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-400">Data Pagamento</p>
                    <p className="text-sm text-white">
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  <div className="flex-1">
                    ...
                    <p className="text-gray-400 mt-2">{rec.descricao}</p>
                  </div>
                </div>
